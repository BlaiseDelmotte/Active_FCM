...
if (SAVE_END == NCYCLEMAX + 1) then
 ...
 if (VORONOI == 1) then
  ...
  if (ERR_FILE_POS.eq.0) then
	  ...
	  do I = PART_START, PART_END
	   write(301,2012) I, (FCM_POS(I,J,1)/RADMAX, J=1,3)
	  end do
  ...
  ! No Periodicity
  if (ERR_FILE_POS_NOPER.eq.0) then
	  !!-Print time
	  write(FILE_EXT,10205) TIME

	  !!-Print filename
	  write(FILENAME,10101)'FCM_PART_POS_NOPER_t',trim(FILE_EXT),'.dat'
	  ...
	  do I = PART_START, PART_END
	   write(301,'(3(e17.7))') (FCM_POS_NOPER(I,J,1), J=1,3)
	  end do
  ...
  !! 7.2 Velocities
  !!--------------------------------------------------------------------
  if (ERR_FILE_VEL.eq.0) then
	  !!-Print time
	  write(FILE_EXT,10205) TIME

	  !!-Print filename
	  write(FILENAME,10101)'FCM_PART_VEL_t',trim(FILE_EXT),'.dat'
	  ...
	  write(302,2003)
	  write(302,2011)
	  write(302,'(I4.4)')PART_END - PART_START +1

	  do I = PART_START, PART_END
	   write(302,'(3(e17.7))') (FCM_VEL(I,J,1), J=1,3)
	  end do
  ...
  !! 7.3 Rotations
  !!--------------------------------------------------------------------
  if (ERR_FILE_ROT.eq.0) then
	  !!-Print time
	  write(FILE_EXT,10205) TIME

	  !!-Print filename
	  write(FILENAME,10101)'FCM_PART_ROT_t',trim(FILE_EXT),'.dat'
	  ...
	  write(303,2004)
	  write(303,2011)
	  write(303,'(I4.4)')PART_END - PART_START +1

	  do I = PART_START, PART_END
	   write(303,'(3(e17.7))') (FCM_ROT(I,J,1), J=1,3)
	  end do
  ...
  !!--------------------------------------------------------------------
  !! 7.4 Quaternions or Orientation Vectors
  !!--------------------------------------------------------------------
  if (USE_QUAT==1) then
   if (ERR_FILE_ORIENT.eq.0) then
	   !!-Print time
	   write(FILE_EXT,10205) TIME

	   !!-Print filename
	   write(FILENAME,10101)'FCM_PART_ORIENT_t',trim(FILE_EXT),'.dat'
	   ...
	   write(304,2005)
	   write(304,2011)
	   write(304,'(I4.4)')PART_END - PART_START +1

	   do I = PART_START, PART_END
	    write(304,'(4(e17.7))') (FCM_ORIENT(I,J,1), J=1,4)
	   end do
   ...
   if (ERR_FILE_P2.eq.0) then
	   !!-Print time
	   write(FILE_EXT,10205) TIME

	   !!-Print filename
	   write(FILENAME,10101)'FCM_PART_P2_t',trim(FILE_EXT),'.dat'
	   ...
	   write(305,2006)
	   write(305,2011)
	   write(305,'(I4.4)')PART_END - PART_START +1

	   do I = PART_START, PART_END
	    write(305,'(3(e17.7))') (FCM_P2(I,J,1), J=1,3)
	   end do

	   !- close file
	   close(305)
   end if
   if (ERR_FILE_P3.eq.0) then
	   !!-Print time
	   write(FILE_EXT,10205) TIME

	   !!-Print filename
	   write(FILENAME,10101)'FCM_PART_P3_t',trim(FILE_EXT),'.dat'
	   ...
	   do I = PART_START, PART_END
	    write(305,'(3(e17.7))') (FCM_P3(I,J,1), J=1,3)
	   end do
 ...
 !!--------------------------------------------------------------------
 !! 7.5 Swimmming direction
 !!--------------------------------------------------------------------
 if (ERR_FILE_SWIM.eq.0) then
	 !!-Print time
	 write(FILE_EXT,10205) TIME

	 !!-Print filename
	 write(FILENAME,10101)'FCM_PART_PSWIM_t',trim(FILE_EXT),'.dat'
	 ...
	 do I = PART_START, PART_END
	  write(305,'(3(e17.7))') (FCM_PSWIM(I,J,1), J=1,3)
	 end do
 ...
 !!--------------------------------------------------------------------
 !! 7.6 Rigidity Stresslet Coefficients
 !!--------------------------------------------------------------------
 if (ERR_FILE_STRESS.eq.0) then
	 !!-Print time
	 write(FILE_EXT,10205) TIME

	 !!-Print filename
	 write(FILENAME,10101)'FCM_PART_STRESSLET_t',trim(FILE_EXT),'.dat'

	 !!- ASCII
	 open(unit=306,file=trim(FILENAME))
	 !!- Ecriture ASCII
	 write(306,2007)
	 write(306,2011)
	 write(306,'(I4.4)')PART_END - PART_START +1

	 do I = PART_START, PART_END
	  write(306,'(5(e17.7))') (FCM_SIJ(I,J,1), J=1,5)
	 end do
...
do IND=1,NB_DUMP_FILES_PART,STEP_SAVE
 ...
 if ( (TIME>=SAVE_START).and.(TIME<=SAVE_END) ) then
  ...
  !!--------------------------------------------------------------------
   ...
   !!--------------------------------------------------------------------
   !! 9.6 Rigidity Stresslet Coefficients
   !!--------------------------------------------------------------------
   if (ERR_FILE_STRESS.eq.0) then
	   !!-Print time
	   write(FILE_EXT,10205) TIME
