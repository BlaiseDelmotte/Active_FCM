...
Program MPIIO_TO_ASCII_POS_NOPER_ONLY
...
if (SAVE_END == NCYCLEMAX + 1) then
 ...
 call READ_VAR_MPIIO(NPART,3,1,FCM_POS_NOPER,FILENAME,ERR_FILE_POS)
 ...
 if (VORONOI == 1) then
  ...
  if (ERR_FILE_POS.eq.0) then
	  ...
	  !!-Print filename
	  write(FILENAME,10101)'FCM_PART_POS_NOPER_VORO_t',trim(FILE_EXT),'.dat'

	  !!- ASCII
	  open(unit=301,file=trim(FILENAME))
	  !!- Ecriture ASCII
	  do I = PART_START, PART_END
	   write(301,2012) I, (FCM_POS_NOPER(I,J,1)/RADMIN, J=1,3)
	  end do
...
end program MPIIO_TO_ASCII_POS_NOPER_ONLY
