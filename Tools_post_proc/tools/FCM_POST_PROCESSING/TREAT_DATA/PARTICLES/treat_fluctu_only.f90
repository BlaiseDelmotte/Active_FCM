...
print*,'NB_DUMP_FILES_PART + 1 -SAVE_START= ' ,  NB_DUMP_FILES_PART + 1-SAVE_START


allocate( TIME_VEC(NB_DUMP_FILES_PART+1-SAVE_START) )

allocate( FCM_RADII(NPART,3,1) )

allocate( FCM_POS(NPART,3,1) )
allocate( FCM_POS_TIME(NB_DUMP_FILES_PART+1-SAVE_START,NPART,3) )
...
if (TREAT_VEL==1) then
 ...
 allocate( FCM_VEL_TIME(NB_DUMP_FILES_PART+1-SAVE_START,NPART,3) )
end if
...
FCM_POS_TIME(NB_DUMP_FILES_PART+1-SAVE_START,:,:) = FCM_POS(:,:,1)
...
if (TREAT_VEL==1) then
 ...
 FCM_VEL_TIME(NB_DUMP_FILES_PART+1-SAVE_START,:,:) = FCM_VEL(:,:,1)
end if

 

TIME_VEC(NB_DUMP_FILES_PART+1-SAVE_START) = (real(NCYCLEMAX)-1.0)*DTIME*VSW/RADMAX
...
do IND= SAVE_START,NB_DUMP_FILES_PART
 K = K+1



 if ((mod(K,(NB_DUMP_FILES_PART-SAVE_START)/5)==0).or.(IND==(NB_DUMP_FILES_PART-SAVE_START))) then
  print*, K, '/', (NB_DUMP_FILES_PART+1-SAVE_START), 'FILES READ'
 end if
 
 TIME = (IND-1)*FOUT3 + 1
 TIME_VEC(K) = real(TIME)*DTIME*VSW/RADMAX
...
if (TREAT_DIFF==1) then
 call VEL_AUTOCORREL(NB_DUMP_FILES_PART+1, SAVE_START, NPART, DTIME, FCM_VEL_TIME)
end if  
