...
subroutine POSITION_VELOCITY_ORIENT_NEMATIC_CORREL(NSAVES, NPART, L, RAD, ROT_MAT, POSI, VEL)
...
!- In(vel,r,theta,phi)
real(kind=8), allocatable, dimension(:,:,:) :: NEMCOR
!- In(r,theta)
real(kind=8), allocatable, dimension(:,:) :: NEMCOR_R_THETA
!- In(r)
real(kind=8), allocatable, dimension(:) :: NEMCOR_R
...
allocate(NEMCOR(NR,NTHETA,NPHI))
allocate(NEMCOR_R_THETA(NR,NTHETA))
allocate(NEMCOR_R(NR))
...
! 2. COMPUTE PDF
!=====================================================================
print*,'COMPUTE RDF AND VELCOR AND POLARCOR AND NEMCOR'
...
!~     IND_VELCOR = int(VELCOR_IJ/DVELCOR)
    ...
    NEMCOR_IJ = 0.5*( 3.0*( ROT_I(3,1)*ROT_J(3,1) &
                            + ROT_I(3,2)*ROT_J(3,2) &
                            + ROT_I(3,3)*ROT_J(3,3) )**2 - 1.0 )   
...
!~         VELCOR_R_PDF(IND_VELCOR,IND_R) + 2
    ...
    POLARCOR(IND_R,IND_THETA_I, IND_PHI_I) = &
        POLARCOR(IND_R,IND_THETA_I, IND_PHI_I) + POLARCOR_IJ
    POLARCOR(IND_R,IND_THETA_J, IND_PHI_J) = &
        POLARCOR(IND_R,IND_THETA_J, IND_PHI_J) + POLARCOR_IJ  
    ...
    NEMCOR(IND_R,IND_THETA_I, IND_PHI_I) = &
        NEMCOR(IND_R,IND_THETA_I, IND_PHI_I) + NEMCOR_IJ
    NEMCOR(IND_R,IND_THETA_J, IND_PHI_J) = &
        NEMCOR(IND_R,IND_THETA_J, IND_PHI_J) + NEMCOR_IJ  

    NEMCOR_R_THETA(IND_R,IND_THETA_I) = &
        NEMCOR_R_THETA(IND_R,IND_THETA_I) + NEMCOR_IJ        
    NEMCOR_R_THETA(IND_R,IND_THETA_J) = &
        NEMCOR_R_THETA(IND_R,IND_THETA_J) + NEMCOR_IJ
        
    NEMCOR_R(IND_R) = &
        NEMCOR_R(IND_R) + 2.0*NEMCOR_IJ
...
POLARCOR = POLARCOR/(NPART*(NSAVES-SAVE_START + 1))
POLARCOR_R_THETA  = POLARCOR_R_THETA /(NPART*(NSAVES-SAVE_START + 1))
POLARCOR_R = POLARCOR_R/(NPART*(NSAVES-SAVE_START + 1))
print*,'maxval(POLARCOR ) = ',maxval(POLARCOR )
print*,'maxval(POLARCOR_R_THETA ) = ',maxval(POLARCOR_R_THETA )
print*,'maxval(POLARCOR_R ) = ',maxval(POLARCOR_R )

NEMCOR = NEMCOR/(NPART*(NSAVES-SAVE_START + 1))
NEMCOR_R_THETA  = NEMCOR_R_THETA /(NPART*(NSAVES-SAVE_START + 1))
NEMCOR_R = NEMCOR_R/(NPART*(NSAVES-SAVE_START + 1))
print*,'maxval(NEMCOR ) = ',maxval(NEMCOR )
print*,'maxval(NEMCOR_R_THETA ) = ',maxval(NEMCOR_R_THETA )
print*,'maxval(NEMCOR_R ) = ',maxval(NEMCOR_R )
...
if (NR*NTHETA*NPHI*4.0*16.0/10.0**6<100.0) then
 ...
 !!-Print filename
 write(FILENAME,10200)'POLARCOR.dat'
 ...
 do I = 1, NR
  do J = 1, NTHETA
   do K = 1, NPHI
    write(301,'(4(e17.7))') &
     RANGE_R(I), RANGE_THETA(J), RANGE_PHI(K), POLARCOR(I,J,K)
   end do
  end do
 end do
...
!---------- POLARcor(r,theta)-----------
!!-Print filename
write(FILENAME,10200)'POLARCOR_R_THETA.dat'
...
do I = 1, NR
 do J = 1, NTHETA
   write(302,'(3(e17.7))') &
    RANGE_R(I), RANGE_THETA(J), POLARCOR_R_THETA(I,J)
 end do
end do
...
!---------- POLARcor(r)-----------
!!-Print filename
write(FILENAME,10200)'POLARCOR_R.dat'
...
if (NR*NTHETA*NPHI*4.0*16.0/10.0**6<100.0) then
 ...
 !---------- NEMcor(r,theta,phi)-----------
 !!-Print filename
 write(FILENAME,10200)'NEMCOR.dat'
 ...
 do I = 1, NR
  do J = 1, NTHETA
   do K = 1, NPHI
    write(301,'(4(e17.7))') &
     RANGE_R(I), RANGE_THETA(J), RANGE_PHI(K), NEMCOR(I,J,K)
   end do
  end do
 end do
 ...
 print*,'SAVE NEMCOR -->  OK '
else
 print*,NR*NTHETA*NPHI*4.0*16.0/10.0**6,' MB TO STORE  , TOO BIG!!!'
end if

print*,'SAVE NEMCOR_R_THETA  '
print*,NR*NTHETA*3.0,' VALUES TO SAVE  '
print*,NR*NTHETA*3.0*16.0/10.0**6,' MB TO STORE  '
!---------- NEMcor(r,theta)-----------
!!-Print filename
write(FILENAME,10200)'NEMCOR_R_THETA.dat'
...
do I = 1, NR
 do J = 1, NTHETA
   write(302,'(3(e17.7))') &
    RANGE_R(I), RANGE_THETA(J), NEMCOR_R_THETA(I,J)
 end do
end do
...
!---------- NEMcor(r)-----------
!!-Print filename
write(FILENAME,10200)'NEMCOR_R.dat'
...
end subroutine POSITION_VELOCITY_ORIENT_NEMATIC_CORREL
