...
if (TREAT_DIFF==1) then
 call SHORT_TIME_SELF_DIFF(NB_DUMP_FILES_PART+1, SAVE_START,1, NPART, DTIME, FCM_VEL_TIME, FCM_ROT_TIME)
end if  

if (TREAT_VEL==1) then
 call PDF_VELOCITIES(NB_DUMP_FILES_PART+1, SAVE_START, NPART, FCM_VEL_TIME)
 call PDF_ROTATIONS(NB_DUMP_FILES_PART+1, SAVE_START, NPART, FCM_ROT_TIME)
 call MEAN_VEL(NB_DUMP_FILES_PART+1,NPART,TIME_VEC,FCM_VEL_TIME)
end if

call POLAR_ORDER(NB_DUMP_FILES_PART+1, &
                 NPART, &
                 NBOX_DIR, &
                 LXMAX, &
                 LYMAX, &
...
!~ if ((TREAT_CORREL_POS==1).and.(TREAT_VEL==1)&
!~                          .and.(TREAT_CORREL_ORIENT==1)&
!~                          .and.(NELLIPSOID>0)) then  
!~                                     
!~  call POSITION_VELOCITY_ORIENT_NEMATIC_CORREL(NB_DUMP_FILES_PART+1, &
!~                                               NPART, &
!~                                               LXMAX, &
!~                                               RADMAX, &
!~                                               FCM_ROT_MAT_TIME, &
...
!~ if ((TREAT_CORREL_POS==1).and.(TREAT_VEL==0)) then
!~ 
!~  call POSITION_CORREL(NB_DUMP_FILES_PART+1, &
!~                       NPART, &
!~                       LXMAX, &
!~                       RADMAX, &
!~                       FCM_ROT_MAT_TIME, &
!~                       FCM_POS_TIME)
!~                       
!~ elseif ((TREAT_CORREL_POS==1).and.(TREAT_VEL==1)&
!~                              .and.(TREAT_CORREL_ORIENT==0)) then  
!~                                   
!~  call POSITION_VELOCITY_CORREL(NB_DUMP_FILES_PART+1, &
!~                               NPART, &
!~                               LXMAX, &
!~                               RADMAX, &
!~                               FCM_ROT_MAT_TIME, &
