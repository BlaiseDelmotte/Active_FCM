...
subroutine POSITION_VELOCITY_ORIENT_CORREL_BIDISPERSE(NSAVES, &
                                                      SAVE_START, &
                                                      PART_START_1, &
                                                      PART_END_1, &
                                                      PART_START_2, &
...
! Number of swimmers
integer, intent(in) :: PART_START_1, PART_END_1, PART_START_2, PART_END_2, NPART
! Number of swimmers to treat
integer, intent(in) :: NPSTAT
! Domain size
real(kind=8), intent(in):: L
! MAximal particle radius
real(kind=8), dimension(NPART), intent(in):: RAD
...
integer, dimension(5*NPART) :: IND_RAND_UNIQUE_1,IND_RAND_UNIQUE_2
...
!- MAx/min radius
real(kind=8) ::TWORAD, RADMAX, TWORADMAX, RADMIN, TWORADMIN, RADMINPLUSMAX
...
real(kind=8), dimension(3) :: POS_IJ
real(kind=8) :: POS_IJ_I, POS_IJ_J
real(kind=8) :: RIJ
real(kind=8), dimension(3) :: PSWIM_I, PSWIM_J
...
!- Indices for loops on particles to chose populations
integer :: NPSTAT_1, NPSTAT_2
...
!~ RADMINPLUSMAX = RADMAX + RADMIN !!!! TO UNCOMMENT!!!!!!!!!
RADMINPLUSMAX = 0.0

MAX_DIST = L_2
print*,'MAX_DIST/RADMIN = ', MAX_DIST/RADMIN

DR = 0.1*RADMAX
NR = ceiling((MAX_DIST - RADMINPLUSMAX)/DR)
...
if ((PART_END_1-PART_START_1+1).le.NPSTAT) then
 NPSTAT_1 = PART_END_1-PART_START_1+1
 K=0
 do I = PART_START_1, PART_END_1
  K = K+1
  IND_RAND_UNIQUE_1(K) = I
 end do
else 
 NPSTAT_1 = NPSTAT
end if

if ((PART_END_2-PART_START_2+1).le.NPSTAT) then
 NPSTAT_2 = PART_END_2-PART_START_2+1
 K=0
 do I = PART_START_2, PART_END_2
  K = K+1
  IND_RAND_UNIQUE_2(K) = I
 end do
else 
 NPSTAT_2 = NPSTAT
end if

print*,'NB OF PART_1 USED TO COMPUTE CORRELATIONS = ', NPSTAT_1
print*,'NB OF PART_2 USED TO COMPUTE CORRELATIONS = ', NPSTAT_2
...
do IND = SAVE_START, NSAVES 
 ...
 if (NPSTAT_1.lt.(PART_END_1-PART_START_1+1)) then   
  ...
  IND_RAND = PART_START_1  + int(real(IND_RAND)*real(PART_END_1-PART_START_1+1)/10.0**9) 
  
  call REMOVE_DUPS(5*NPART,IND_RAND,NUNIQUE,IND_RAND_UNIQUE_1)
  if (NUNIQUE<NPSTAT_1) then
   print*,'NUNIQUE = ', NUNIQUE
   print*,'Too much repeated elements!!!'
  end if 
 end if
 
 if (NPSTAT_2.lt.(PART_END_2-PART_START_2+1)) then   
  call init_random_seed()
  call random_number(VEC_RAND)
  ...
  IND_RAND = PART_START_2  + int(real(IND_RAND)*real(PART_END_2-PART_START_2+1)/10.0**9) 
  
  call REMOVE_DUPS(5*NPART,IND_RAND,NUNIQUE,IND_RAND_UNIQUE_2)
  if (NUNIQUE<NPSTAT_1) then
   print*,'NUNIQUE = ', NUNIQUE
   print*,'Too much repeated elements!!!'
  end if 
...
!  print*,'IND_RAND_UNIQUE_1(1:NPSTAT) = ', IND_RAND_UNIQUE_1(1:NPSTAT_1)
!  print*,'maxval(IND_RAND_UNIQUE_1(1:NPSTAT_1)) = ', maxval(IND_RAND_UNIQUE_1(1:NPSTAT_1))
!  print*,'minval(IND_RAND_UNIQUE_1(1:NPSTAT_1)) = ', minval(IND_RAND_UNIQUE_1(1:NPSTAT_1))
!  read(*,*)
!  print*,'IND_RAND_UNIQUE_2(1:NPSTAT_2) = ', IND_RAND_UNIQUE_2(1:NPSTAT_2)
!  print*,'maxval(IND_RAND_UNIQUE_2(1:NPSTAT_2)) = ', maxval(IND_RAND_UNIQUE_2(1:NPSTAT_2))
!  print*,'minval(IND_RAND_UNIQUE_2(1:NPSTAT_2)) = ', minval(IND_RAND_UNIQUE_2(1:NPSTAT_2))
!  read(*,*)
...
!~  POLARCOR = 0.0
 ...
 do IND_I = 1, NPSTAT_1
  ...
  I = IND_RAND_UNIQUE_1(IND_I)
  
  if ((I<1).or.(I>PART_END_1-PART_START_1+1)) then
   print*,'IND_I = ', IND_I
   print*,'NUNIQUE = ', NUNIQUE
   print*,'I = ', I
   read(*,*)
...
!~   ROT_I = ROT_MAT(IND,I,1:3,1:3)
  ...
  do IND_J = 1, NPSTAT_2
   ...
   J = IND_RAND_UNIQUE_2(IND_J)   
  
   POS_J = POSI(IND,J,1:3)
   PSWIM_J = PSWIM(IND,J,1:3)
!~    ROT_J = ROT_MAT(IND,J,1:3,1:3)
   ...
   if ((RIJ>=RADMINPLUSMAX).and.(RIJ<MAX_DIST)) then
   
!~     IND_R = ceiling( ( RIJ-RANGE_R(1) )/DR ) + 1
    IND_R = ceiling( ( RIJ-RANGE_R(1) )/DR ) 
        
    POS_IJ_I = POS_IJ(1)*PSWIM_I(1) &
             + POS_IJ(2)*PSWIM_I(2) &
             + POS_IJ(3)*PSWIM_I(3)
...
!~     PHI_I = real(atan2(POS_IJ_I(2),POS_IJ_I(1)))
    THETA_I = real(acos(POS_IJ_I/RIJ))
...
!~     IND_VELCOR = int(VELCOR_IJ/DVELCOR)
    ...
    POLARCOR_IJ = PSWIM_J(1)*PSWIM_I(1) &
                + PSWIM_J(2)*PSWIM_I(2) &
                + PSWIM_J(3)*PSWIM_I(3)
...
!         POLARCOR_R_THETA(IND_R,IND_THETA_J) + POLARCOR_IJ 
    ...
    POLARCOR_R(IND_R) = &
        POLARCOR_R(IND_R) + 1.0*POLARCOR_IJ  
!~     POLARCOR_R(IND_R) = &
!~         POLARCOR_R(IND_R) + POLARCOR_IJ
  ...
  end do   !do IND_J = 1, NPSTAT_2

 end do !do IND_I = 1, NPSTAT_1
...
!~  
!~  CORREL_INTEGRAL_MEAN_TIME(M,1:NR) = CORREL_INTEGRAL_MEAN_TIME(M,1:NR)/real(NPSTAT_1*(NPSTAT-1))
!~  
...
!~  
 ...
 RDF_R_THETA = RDF_R_THETA/(real(NPSTAT_1*NPSTAT_2)/L**(3)*VOL_RING)
 RDF_R = RDF_R/(real(NPSTAT_1*NPSTAT_2)/L**(3)*VOL_SHELL)
...
write(FILE_EXT3,10205) PART_START_1
write(FILE_EXT4,10205) PART_END_1


write(FILENAME,10103) 'RDF_R_THETA_PART_',trim(FILE_EXT3),'_',trim(FILE_EXT4),&
...
!---------- g(r,theta,phi)-----------
!!-Print filename
write(FILENAME,10103) 'RDF_R_PART_',trim(FILE_EXT3),'_',trim(FILE_EXT4),&
                      '_SAVE_',trim(FILE_EXT1),'_',trim(FILE_EXT2),'.dat'
...
!---------- velcor(r,theta)-----------
!!-Print filename
write(FILENAME,10103) 'VELCOR_R_THETA_PART_',trim(FILE_EXT3),'_',trim(FILE_EXT4),&
                      '_SAVE_',trim(FILE_EXT1),'_',trim(FILE_EXT2),'.dat'
...
!---------- velcor(r)-----------
!!-Print filename
write(FILENAME,10103) 'VELCOR_R_PART_',trim(FILE_EXT3),'_',trim(FILE_EXT4),&
                      '_SAVE_',trim(FILE_EXT1),'_',trim(FILE_EXT2),'.dat'
...
!---------- POLARcor(r,theta)-----------
!!-Print filename
write(FILENAME,10103) 'POLARCOR_R_THETA_PART_',trim(FILE_EXT3),'_',trim(FILE_EXT4),&
                      '_SAVE_',trim(FILE_EXT1),'_',trim(FILE_EXT2),'.dat'
...
!---------- POLARcor(r)-----------
!!-Print filename
write(FILENAME,10103) 'POLARCOR_R_PART_',trim(FILE_EXT3),'_',trim(FILE_EXT4),&
                      '_SAVE_',trim(FILE_EXT1),'_',trim(FILE_EXT2),'.dat'
...
end subroutine POSITION_VELOCITY_ORIENT_CORREL_BIDISPERSE
